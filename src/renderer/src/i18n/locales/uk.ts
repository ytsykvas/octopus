...
export const uk: Translation = {
  ...
  project: {
    ...
    envHint:
      'По одному KEY=value в рядку. Дописуються в кінець .env воркспейсу, тож перебивають те, що скопіювалося, — і стають усім файлом там, де клонові не було чого копіювати. $OCTOPUS_PORT замінюється на власний порт воркспейсу, тож значення з портом у кожного своє. Лишаються на цій машині, у репозиторій не потрапляють.',
    envNotIgnored:
      'git не ігнорує {{file}} у цьому репозиторії, тож усе, що тут введено, з’явиться у воркспейсі як зміна — і може потрапити в коміт. Додайте його до .gitignore.',
