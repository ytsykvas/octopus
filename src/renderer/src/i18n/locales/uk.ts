...
export const uk: Translation = {
  ...
  project: {
    ...
    envHint:
      'По одному KEY=value в рядку. Дописуються в кінець .env воркспейсу, тож перебивають те, що скопіювалося, — і стають усім файлом там, де клонові не було чого копіювати. Лишаються на цій машині, у репозиторій не потрапляють.',
