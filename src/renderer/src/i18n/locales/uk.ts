...
export const uk: Translation = {
  ...
  repositories: {
    ...
    silentOrganisations:
      'Ви належите до {{names}}, і жодного репозиторію звідти в списку немає. Або там немає нічого, куди ви можете пушити, або токен потребує SSO-авторизації для цієї організації на GitHub.',
  ...
  project: {
    ...
    declaredFiles: 'Оголошено репозиторієм',
    declaredFilesHint:
      '{{path}} перелічує файли, які цей репозиторій вважає потрібними своїм робочим зонам. octopus не діє за цим переліком — переносить саме той список, що вище, — але може перенести записи туди.',
    declaredCarried: 'уже в списку',
    declaredPattern: 'шаблон — octopus копіює лише названі файли',
    declaredAdd_one: 'Додати {{count}} запис до списку вище',
    declaredAdd_few: 'Додати {{count}} записи до списку вище',
    declaredAdd_many: 'Додати {{count}} записів до списку вище',
    declaredAdd_other: 'Додати {{count}} записів до списку вище',
    ...
    envProfileName: 'Назва набору (малі літери, цифри й дефіси)',
    envProfileNewTitle: 'Новий набір змінних',
    envProfileDuplicateTitle: 'Копія «{{name}}»',
    envProfileRenameTitle: 'Перейменувати «{{name}}»',
    envProfileNameConfirm: 'Створити',
    envProfileRenameConfirm: 'Перейменувати',
    envProfileNameCancel: 'Скасувати',
    envProfileNameInvalid: 'Лише малі літери, цифри й дефіси; починатися з літери або цифри.',
    envProfileNew: 'Новий',
    envProfileDuplicate: 'Копія',
    envProfileRename: 'Перейменувати',
  ...
  scripts: {
    ...
    repoCommandNotice:
      'Один із них — це командний рядок, а не файл. Дозволяючи його, ви дозволяєте сам рядок — а не те, що він запускає: того тут не показано, і при зміні воно не спитає знову.',
    ...
    carryMissing:
      'Не скопійовано, тож скрипти працюють без них: {{files}}. Перевірте список файлів \u2014 джерело могло переїхати, або цей клон їх ніколи не мав.',
    runAll: 'Запустити',
    running: 'Збирається\u2026',
  ...
  pullRequest: {
    capped:
      'GitHub запитали про сто найновіших коментарів і п’ятдесят гілок обговорення — і він відповів рівно стількома, тож старіші сюди могли не потрапити. На GitHub у запиті є всі.',
    ...
    mergeChecksRunning_one: '{{count}} перевірка ще виконується.',
    mergeChecksRunning_few: '{{count}} перевірки ще виконуються.',
    mergeChecksRunning_many: '{{count}} перевірок ще виконуються.',
    mergeChecksRunning_other: '{{count}} перевірок ще виконуються.',
    mergeChecksFailed_one: 'Впала {{count}} перевірка. Полагодьте її перед злиттям.',
    mergeChecksFailed_few: 'Впали {{count}} перевірки. Полагодьте їх перед злиттям.',
    mergeChecksFailed_many: 'Впало {{count}} перевірок. Полагодьте їх перед злиттям.',
    mergeChecksFailed_other: 'Впало {{count}} перевірок. Полагодьте їх перед злиттям.',
    ...
    push: 'Запушити',
    pushing: 'Пушимо…',
    unpushed_one: '{{count}} коміт ще не на GitHub.',
    ...
    commitNeedsMessage: 'Спершу напишіть повідомлення коміта вище.',
    commitAndPushHint:
  ...
  diff: {
    ...
    modeExecutable: 'став виконуваним',
    modeNotExecutable: 'більше не виконуваний',
    modeChanged: 'режим {{from}} \u2192 {{to}}',
    ...
    publishUncommitted: 'не закомічено',
    publishCommitted: 'закомічено',
    publishPushed: 'запушено',
    publishUncommittedLabel: 'Не закомічено — закомітьте, тоді запуште',
    publishCommittedLabel: 'Закомічено тут, на GitHub ще немає — запуште',
    ...
    publishUnpushed_one: '{{count}} коміт не запушено',
    publishUnpushed_few: '{{count}} коміти не запушено',
    publishUnpushed_many: '{{count}} комітів не запушено',
    publishUnpushed_other: '{{count}} комітів не запушено',
    publishStaleCount_one: '{{count}} файл у пул-реквесті застарів',
    publishStaleCount_few: '{{count}} файли у пул-реквесті застаріли',
    publishStaleCount_many: '{{count}} файлів у пул-реквесті застаріли',
    publishStaleCount_other: '{{count}} файлів у пул-реквесті застаріли',
  ...
  chat: {
    ...
    compacted: 'Агент підсумував усе вище, щоб звільнити місце',
    compactedBy: 'Агент підсумував усе вище, щоб звільнити місце — {{before}} стало {{after}}',
    refusalSwapped:
      '{{from}} відмовилася від цього ходу, тож його взяла {{to}} — і решту цієї розмови теж',
    refusalSwappedLocally:
      '{{from}} відмовилася від частини цього ходу, тож ту частину відповіла {{to}}',
    refusalDeclined: '{{from}} відмовилася від цього ходу, і жодна інша модель його не взяла',
    retracted: 'Агент забрав це назад після того, як модель відмовилася від ходу.',
  ...
  skills: {
    ...
    renameLabel: 'Нова назва (малі літери, цифри й дефіси)',
    renameConfirm: 'Перейменувати',
    renameCancel: 'Скасувати',
    renameInvalid: 'Лише малі літери, цифри й дефіси; починатися з літери або цифри.',
    remove: 'Видалити',
  ...
  library: {
    ...
    subagents: 'Субагенти',
    subagentsNote:
      'Помічники, до яких агент звертається сам, кожен зі своїми інструкціями. Зберігаються в octopus, а не в репозиторії. Новий побачать розмови, розпочаті після нього, а не ті, що вже тривають.',
    subagentsProjectNote:
      'Помічники, до яких агент звертається в цьому проєкті, кожен зі своїми інструкціями. Зберігаються в octopus, а не в репозиторії. Новий побачать розмови, розпочаті після нього, а не ті, що вже тривають.',
  ...
  trust: {
    ...
    review: 'Переглянути',
    linkRefused: 'Посилання за межі цієї робочої копії. Його показано, а не прочитано: {{target}}',
  ...
  errors: {
    ...
    draftUnreadable:
      'Агент відповів, але не заголовком і описом. Спробуйте ще раз або напишіть їх самі.',
    envProfileExists:
    ...
    envPathEscapes:
      '{{file}} виходить за межі воркспейса через символічне посилання, тож змінні записалися б поза ним. Вкажіть проєкту файл, який справді є у ворктрі, або приберіть посилання.',
    ...
    slugTaken:
      'Інший воркспейс, «{{name}}», уже дає базі даних імʼя {{slug}}. Два воркспейси з однією базою перезаписують дані одне одного. Оберіть імʼя, що відрізняється більше, ніж дефісом чи крапкою.',
