...
| [`usage.ts`](../src/core/usage.ts)                         | what `/usage` answers, narrowed from a response wider than its own type — and the one narrowing of it: the sidebar's windows come out of the same report rather than a second reader of the same request                                                 |
...
| [`conductorConfig.ts`](../src/core/conductorConfig.ts) | a repository's `.conductor/` settings, as far as octopus can use them: three command lines and four prompts. **Not** its `file_include_globs` — that was parsed here and read by nobody, while this row and two comments said otherwise; the carry list is deliberately not live (`docs/repo-config.md`), and a glob could not be followed anyway |
...
| [`diff.ts`](../src/core/diff.ts)             | what a workspace changed, and parsing what git prints. **Three reads, not two**: `--numstat` for the counts, `--name-status` for the kind, and `--raw` for the file modes — the last because a unified diff cannot be relied on for a mode, a permission change having no hunks at all and one beside content changes sitting above `@@` where the parser never looks. `--raw` is exact for every file whatever the drawing budget, which a mode carried by the unified parser would not be |
