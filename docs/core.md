...
| [`conductorConfig.ts`](../src/core/conductorConfig.ts) | a repository's `.conductor/` settings, as far as octopus can use them: three command lines, four prompts, and the `file_include_globs` it declares — read and **shown**, never followed. The carry list is deliberately not live (`docs/repo-config.md`) and a glob could not be copied anyway, so what the parse is for is telling the reader what the repository declares beside the list that decides |
