...
- A project also holds **env overrides**: `KEY=value` lines typed in its settings and appended to every workspace's env file — `.env` unless the project names another, because Vite and Next read `.env.local` — between markers so a change replaces rather than piles up. Last wins, which is the point twice over — it overrides a checkout left pointing at production, and it is the whole file for a project cloned from GitHub, where a fresh clone has no gitignored `.env` to carry in the first place. They are appended only: nothing exports them into the shell, so the terminal sees them just through whatever reads `.env`.
- Both scripts are given `$OCTOPUS_ROOT_PATH` (the checkout) and `$OCTOPUS_WORKSPACE_NAME`; only the server also gets `$OCTOPUS_PORT`.
- **A repository may carry a copy of all of it.** Everything above lives under `~/.octopus`, which is one directory on one machine; a project's repository can hold the same scripts, carry list, instructions and fields in `.octopus/`, so a wiped installation is rebuilt from the repository rather than from memory. Insurance, and nothing else: a project whose repository has none behaves exactly as before.
- **The copy is a snapshot, and nothing reads it while the app works.** It moves by two actions in Project settings — Import, which shows every byte of a file before writing it, and Export, which writes the directory from what the app holds. Reading it live would mean running shell that arrived with a `git pull`, and approval here is a fact about a **worktree**: a project re-added after a wipe has approved nothing and has no workspace to approve in, so a live layer would be off in the one situation it exists for. Which side is newer is deliberately never guessed — contents are all there is to go on after a clone, so the panel says the two differ and offers both directions.
...
~/.octopus/
  ...
  projects/<slug>/
    ...
    env                      variables appended to every workspace's .env
    scripts/setup.sh
    scripts/run.sh
    scripts/archive.sh
