...
| `projects:listRemote`    | —             | what the account can push to, personal and organisation alike, through `gh api graphql`, whether the walk stopped at its limit, and which organisations the account belongs to — the last a sibling field on the same `viewer`, so it costs no second request and is what lets the picker name an organisation that put nothing in the list |
...
| `workspace:prepare`      | `workspaceId`         | carries the listed files in, never over a file already there, then writes the env block below them; answers `{ written, missing }` — the second being what the list named and the worktree still has not got, which is a notice on the run rather than a failure of it  |
...
| `scripts:project`        | `id`, `workspaceId`   | the same answer for a project rather than a worktree, so the settings dialog can say which script would actually run. The workspace is asked about as well where there is one — a branch may carry a script the checkout has not — and `null` answers from the checkout |
...
| `skills:rename`       | `store`, `name`, `to`      | a **migration**, not an edit: the directory moves, the frontmatter's `name:` follows it, and the key moves in all three places it is stored — the installation's default list, every project's, and every chat's overrides. Left behind, they would switch the skill back on wherever it had been turned off, since a key no list mentions reads as on                                                                                                                    |
| `skills:inspect`      | `store`, `request`         | what an import would write, without writing it. The link is what it is for: a folder the user picked they know and pasted text they can read, while an address shows nothing until it is fetched — and what comes back is prose the agent will later follow. The answer carries the document back for that route alone, so importing what was previewed does not ask the address twice: the second answer need not be the first, and it is the first the reader agreed to |
...
| `workspaces:diff`                | `id`                                  | everything changed since the base branch, each file stamped with how far its change has got towards GitHub and whether the request shows an older version of it (`publish.ts`). One payload rather than a second call: two reads can describe two different trees, and a mark for a file the list no longer has is worse than no mark. Carries four facts about the branch beside the files: whether the worktree has the workspace's own branch checked out — false suppresses every claim about GitHub, since all of them are measured from `HEAD` — whether the repository has an `origin` at all, the tip of the branch's copy on it, and whether anything is left to send — the last measured against the remote rather than counted from the files, because a change that nets out against the base leaves the list while still being unsent |
| `workspaces:fileSides`           | `id`                                  | the same files' two sides **complete**, which is what the syntax colours need and the drawn diff does not have. A `Record` rather than the `Map` it is built as, because structured cloning keeps a `Map` and the preload's own types would then be the only place saying so                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `workspaces:pullRequestDetail`   | `workspaceId`, `number`               | the checks, the review and the mergeability of a request that exists — `gh pr view` plus a GraphQL query for the review threads. Apart from the read above because the two fail differently: that one failing means the branch cannot be described at all, this one leaves the number and the link worth drawing . Says whether a read came back **full**: `gh pr view` sends at most a hundred comments and a hundred reviews, and the threads query asks for fifty with twenty replies each, so the pane can say that what it draws is not all there is rather than leaving it to be discovered                                                                                                                                                                                                                                                  |
| `workspaces:draftPullRequest`    | `workspaceId`                         | asks the agent for a title and a description, under the project's own instruction; stops at producing text, which the form then shows before anything reaches GitHub. The prompt says what the task is **not** before handing that instruction over — nothing is run, committed, pushed or opened here, and the tools are read-only — because a project's instruction is written for the whole act of opening a request and a good one says to run its checks first. Left unframed, an agent took that literally, spent its turn failing to run a test suite, and answered with no title at all                                                                                                                                                                                                                                                    |
...
| `workspaces:push`                | `workspaceId`                         | sends what is **already committed** and leaves uncommitted work where it is. The other half of the pair above, and the one that was missing: work the agent committed, or the reader committed in the workspace's terminal, had no way onto the request short of the terminal. Nothing to validate past the id — the branch and the remote are read from the workspace                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `workspaces:mergePullRequest`    | `workspaceId`, `number`, `method`     | `merge`, `squash` or `rebase`. Never deletes the branch: a worktree is checked out on it. Answers with nothing — `gh` enables auto-merge rather than merging when a required check has not passed, so the caller reads the request again. The number is checked against the workspace's own branch before anything is merged: the window is not trusted to have kept the pair together, having failed to for one release                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `workspaces:replyToReviewThread` | `workspaceId`, `threadId`, `body`     | answers one review thread. The thread's node id and the body both become arguments to `gh`, so both are validated here rather than trusted because the renderer read them from us                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `workspaces:resolveReviewThread` | `workspaceId`, `threadId`, `resolved` | settles a thread or puts it back. Both directions: undoing one is a click on GitHub, and a pane that can do a thing but not undo it sends the reader to the browser anyway                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
