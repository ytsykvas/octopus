...
| `chats:effort`          | `chatId`, `effort`                 | same shape as the mode; refuses `null`, which used to mean "leave it to the agent" — a chat always names a level now, and the level shown is the level sent                                                                                  |
