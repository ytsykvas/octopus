...
of 41 channels. The table lives in [`src/main/ipc.ts`](../src/main/ipc.ts); the
...
| `chats:effort`     | `chatId`, `effort`                 | same, but `null` only reaches the record — a session already started cannot be handed back to the agent's own judgement                                                                             |
| `chats:model`      | `chatId`, `model`                  | same shape as the mode; `null` hands the choice back to the agent                                                                                                                                   |
