...
- `src/core/pullRequests.ts` — `BRANCH_REQUEST_LIMIT`, and `THREADS_QUERY`'s
  `first: 50` / `first: 20`.
- `src/renderer/src/components/pr/CommentList.tsx` draws what it is given and
  has no notion of there being more.

## What is already decided

- A cap rather than pagination, for the reason `REPOSITORY_LIMIT` gives: past
  this the answer needs a search box rather than a longer page.
